...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return batch_entities, batch_images, batch_y
...
class BertClassifier_Update(nn.Module):
    ...
    def __init__(self,bert_model, dropout=0.5):
        ...
        self.fc1 = nn.Linear(self.entity.config.hidden_size,512)
        self.fc3 = nn.Linear(512,4)
        self.mm =fusions.Block([512,512], 512)
        self.attention=Attention(512)


    def forward(self, input_id_entity, mask_entity, img):
        #print(input_id.shape)
        #print(mask.shape)
        
        _, pooled_output_entity = self.entity(input_ids= input_id_entity, attention_mask=mask_entity,return_dict=False)
        entity_out = self.fc1(pooled_output_entity)
        img_out = self.vit(img)

        attn_entity= entity_out.reshape(-1,1,entity_out.shape[1])
        attn_img= entity_out.reshape(-1,1,img_out.shape[1])

        output, weights=self.attention(attn_entity,attn_img)
...
def eval(model, dataloader, criterion, device, save_prediction=False):
    ...
    with torch.no_grad():
        ...
        for val_entity, image, val_label in dataloader:
            ...
            output = model(input_id_entity,mask_entity, image)
            if save_prediction:
            ...
            try:
                ...
                #print("Exveption output shape:", output.shape, "val_label: ", val_label.shape)
                print(e)
...
def train(args):
    ...
    for epoch_num in range(0, epochs):
        ...
        for train_entity, image, train_label in tqdm(train_dataloader):
